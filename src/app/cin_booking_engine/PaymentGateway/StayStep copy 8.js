   ...
   // setIsTaxCalculated(false);
    const dayRate = rateResponse?.Rooms?.flatMap((room) => {
      ...
      return newSelectedRooms.map((selected) => {
        ...
        let obp = dateData.OBP[String(selected.adults)];
...
const extraChildren = 
  adjustedChildren > applicableChild
    ...
    : 0;
        ...
        // try adult-specific OBP
        let obp1 = dateData.OBP[String(selected.adults)];
            ...
            <div className="pl-2">
              <h4 className="wizard-title-main cart-display-mobile" onClick={()=>setCartDisplayMobile(!cartDisplayMobile)}>
                <span className="d-flex w-100 justify-content-between align-items-center">
                 Booking Details <strong className="amount-in-wizard total-amount-cart text-success"> {" - You Saved INR " } {Math.round(totalSavings)?.toLocaleString()}</strong>   {cartDisplayMobile ? (<ChevronUp size={20} />) : (<ChevronDown size={20} />)}
                </span> 
              </h4>  
              <h4 className="wizard-title-main cart-display-desktopss">
                {/* Booking Details {" - Total Savings INR " } {totalSavings} */}
                 Booking Details <strong className="amount-in-wizard total-amount-cart text-success"> {" - You Saved INR " } {Math.round(totalSavings)?.toLocaleString()}</strong> 
              </h4>  
            </div>
