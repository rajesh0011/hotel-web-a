...
const calculateBasePrice = () => {
  return selectedRoom?.reduce((sum, room) => {
    ...
    const roomTotal = Object.values(room.packageRateList).reduce(
      (roomSum, dateData) => {
        const obp1 = dateData?.OBP?.[room?.adults?.toString()] || dateData?.OBP?.[(room?.adults - 1)?.toString()];
  ...
  const rateUpdate = async () => {
    ...
    const dayRate = rateResponse?.Rooms?.flatMap((room) => {
      ...
      return newSelectedRooms.map((selected) => {
        ...
        const taxesFromRates = rateEntries.flatMap(dateData =>
            dateData?.OBP?.[String(selected?.adults)]?.Tax ||  dateData?.OBP?.[String(selected?.adults-1)]?.Tax || []
          );
          
          const rates = matchedRatePlan?.Rates?.[Object.keys(matchedRatePlan?.Rates || {})[0]]?.OBP;
          const primary = rates?.[room?.adults?.toString()]?.RateAfterTax ?? undefined; // [web:21][web:32]
            const fallback = rates?.[(room?.adults - 1)?.toString()]?.RateAfterTax ?? 0;
          ...
          prev.map((rm) =>
            ...
            rm?.roomId === selected?.roomId
              ? {
                  ...rm,
                  packageRate:
                    firstRateObj?.OBP?.[rm?.adults?.toString()]?.RateBeforeTax || firstRateObj?.OBP?.[(rm?.adults-1)?.toString()]?.RateBeforeTax,
                  roomRateWithTax:
                    firstRateObj?.OBP?.[rm?.adults?.toString()]?.RateAfterTax || firstRateObj?.OBP?.[(rm?.adults-1)?.toString()]?.RateAfterTax,
                  adultRate:
    ...
    //     ];
        const taxArray = [
      // collect all OBP taxes for selected.adults across all dates
      ...taxesFromRates,
      ...extraChildTaxes,
      ];
        const taxObject = {};
        ...
        if (firstRateObj && selected) {
          ...
          taxObject["ExtraChildRate"] = totalExtraChildRate * extraChildren;
          }
                  ...
                  packageRate: isMemberRate
                    ? (Math.round(
                        firstRateObj?.OBP?.[rm?.adults?.toString()]?.RateBeforeTax
                         || firstRateObj?.OBP?.[(rm?.adults-1)?.toString()]?.RateBeforeTax
                      )) * 0.9
                    : Math.round(
                        firstRateObj?.OBP?.[rm?.adults?.toString()]?.RateBeforeTax
                         || firstRateObj?.OBP?.[(rm?.adults-1)?.toString()]?.RateBeforeTax
