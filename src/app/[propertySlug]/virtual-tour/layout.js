...
export async function generateMetadata({ params }) {
  ...
  const fallbackMeta = {
    title: "Virtual Tour | Amritara Hotels and Resorts",
    description: "Virtual Tour | Amritara Hotels and Resorts",
  };
  ...
  try {
    ...
    return {
      ...
      alternates: { canonical: `/${propertySlug}/virtual-tour` },
    };
  } catch (err) {
    console.error("Virtual Tour metadata fetch error:", err);
    return fallbackMeta;
  }
}

export default function EVirtualTourLayout({ children }) {
  return children;
}
